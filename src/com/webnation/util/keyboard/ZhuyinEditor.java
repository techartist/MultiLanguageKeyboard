...
public class ZhuyinEditor extends Editor {
	...
	/**
	 * Decomposes the composing-text into four parts: initials, yi-wu-yu-finals, other-finals, and tones, for example, { 'ㄅ', 'ㄨ', 'ㄚ' , 'ˋ' }
	 * 
	 * @return an array of four characters as { initials, yi-wu-yu-finals, other-finals, tones }; any element could be null character if the corresponding part
	...
	public boolean doCompose(int keyCode) {
		...
		if (ZhuyinTable.isTone(c)) {
			...
			if (c == ZhuyinTable.DEFAULT_TONE) {
				if (tone != ZhuyinTable.DEFAULT_TONE) {
					Log.d("", "composingText Pre2: " + composingText);
			...
			if ((composingText.length() <= 0) || (ZhuyinTable.getInitials(composingText.charAt(0)) == 0)) {
