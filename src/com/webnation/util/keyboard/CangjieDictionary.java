...
public class CangjieDictionary extends WordDictionary {
	...
	public String getWords(CharSequence input) {
		...
		return searchWords(secondaryIndex, data, input);
	}
	...
	private String searchWords(int secondaryIndex, char[] data, CharSequence input) {
		int length = data.length / 2;
		...
		String returnableTemp = String.valueOf(data, start + length, end - start);
		String returnable = "";

		for (int j = 0; j < returnableTemp.length(); j++) {
			returnable += returnableTemp.charAt(j) + "|";
		}
	 ...
	 * @return -1 if the value isn't found. TODO: Remove this once Arrays binarySearch supports search within a range.
	 */
	private static int binarySearch(char[] array, int fromIndex, int toIndex, char value) {
