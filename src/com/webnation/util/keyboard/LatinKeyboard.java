...
public class LatinKeyboard extends Keyboard {
	...
	public LatinKeyboard(Context context, int xmlLayoutResId, int mode, float kbHeightPercent) {
		...
		mIsAlphaKeyboard = xmlLayoutResId == R.xml.kbd_qwerty_english;
	...
	public boolean setShiftState(int shiftState) {
		if (mShiftKey != null) {
			...
			mShiftKey.icon = (shiftState == SHIFT_OFF || shiftState == SHIFT_ON || shiftState == SHIFT_LOCKED) ? mOldShiftIcon : mShiftLockIcon;
	...
	// key.modifier = true;
	// if (key.label != null) {
	// key.popupCharacters = (key.popupCharacters == null) ? key.label : key.label + key.popupCharacters.toString();
	// }
	// }
	// key.label = null;
	...
	private void setSettingsF1Key(Key key) {
		...
		final Drawable settingsHintDrawable = new BitmapDrawable(mRes, drawSynthesizedSettingsHintImage(key.width, key.height, mSettingsIcon, mHintIcon));
		key.label = null;
	...
	private static String layoutSpaceBar(Paint paint, Locale locale, Drawable lArrow, Drawable rArrow, int width, int height, float origTextSize,
	...
	private Bitmap drawSpaceBar(int opacity, boolean isAutoCompletion) {
		...
		if (mLocale != null) {
			...
			final String language = layoutSpaceBar(paint, locale, mButtonArrowLeftIcon, mButtonArrowRightIcon, width, height,
					getTextSizeFromTheme(android.R.style.TextAppearance_Small, 14), allowVariableTextSize);
	...
	private int getSpacePreviewWidth() {
		final int width = Math.min(Math.max(mSpaceKey.width, (int)(getMinWidth() * SPACEBAR_POPUP_MIN_RATIO)),
	...
	public void setLanguageSwitcher(LanguageSwitcher switcher, boolean isAutoCompletion) {
		...
		if (locale != null && mLanguageSwitcher.getLocaleCount() == 1
				&& mLanguageSwitcher.getSystemLocale().getLanguage().equalsIgnoreCase(locale.getLanguage())) {
			locale = null;
		}
		mLocale = locale;
	...
	/**
	 * Animation to be displayed on the spacebar preview popup when switching languages by swiping the spacebar. It draws the current, previous and next
	 * languages and moves them by the delta of touch movement on the spacebar.
	 */
