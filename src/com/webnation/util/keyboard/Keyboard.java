...
public class Keyboard {
	...
	public static final int KEYCODE_JA_HIRAGANA = -144;
	public static final int KEYCODE_JA_KATAKANA = -145;
	public static final int KEYCODE_JA_KANJI = -146;
	public static final int KEYCODE_COMPOSING_OK = -147;
	public static final int KEYCODE_CH_PINYIN = -148;
	public static final int KEYCODE_CH_WUBI = -149;
	public static final int KEYCODE_OPEN = -150;
	public static final int KEYCODE_EXTRA_KBD1 = -151;
	public static final int KEYCODE_EXTRA_KBD2 = -152;
	public static final int KEYCODE_EXTRA_KBDRETURN = -153;
	...
	/**
	 * Container for keys in the keyboard. All keys in a row are at the same Y-coordinate. Some of the key size defaults can be overridden per row from what the
	 * {@link Keyboard} defines.
	 * 
	 * @attr ref android.R.styleable#Keyboard_keyWidth
	 * @attr ref android.R.styleable#Keyboard_keyHeight
	 * @attr ref android.R.styleable#Keyboard_horizontalGap
	...
	public static class Key {
		...
		/**
		 * Flags that specify the anchoring to edges of the keyboard for detecting touch events that are just out of the boundary of the key. This is a bit mask
		 * of {@link Keyboard#EDGE_LEFT}, {@link Keyboard#EDGE_RIGHT}, {@link Keyboard#EDGE_TOP} and {@link Keyboard#EDGE_BOTTOM}.
		 */
		...
		private final static int[] KEY_STATE_PRESSED_LOCK = { android.R.attr.state_active, android.R.attr.state_pressed, android.R.attr.state_checkable,
		 ...
		 * @param parent the row that this key belongs to. The row must already be attached to a {@link Keyboard}.
		...
		public Key(Resources res, Row parent, int x, int y, XmlResourceParser parser) {
			...
			if (shiftLabel != null && shiftLabel.length() == 0)
				shiftLabel = null;
			capsLabel = a.getText(R.styleable.Keyboard_Key_capsLabel);
			if (capsLabel != null && capsLabel.length() == 0)
				capsLabel = null;
		...
		private String getPopupKeyboardContent(boolean isShiftCaps, boolean isShifted, boolean addExtra) {
			...
			// Remove duplicates
			if (shiftChar == mainChar)
				shiftChar = 0;
			if (capsChar == shiftChar || capsChar == mainChar)
				capsChar = 0;
		...
		public Keyboard getPopupKeyboard(Context context, int padding) {
			...
			if (popup.length() > 0) {
				...
				if (resId == 0)
					resId = R.xml.kbd_popup_template;
		 ...
		 * @return whether or not the point falls inside the key. If the key is attached to an edge, it will assume that all points between the key and the edge
		...
		public boolean isInside(int x, int y) {
			...
			if ((x >= this.x || (leftEdge && x <= this.x + this.width)) && (x < this.x + this.width || (rightEdge && x >= this.x))
					&& (y >= this.y || (topEdge && y <= this.y + this.height)) && (y < this.y + this.height || (bottomEdge && y >= this.y))) {
				return true;
			} else {
				return false;
			}
		...
		public String toString() {
			...
			String edges = (((edgeFlags & Keyboard.EDGE_LEFT) != 0 ? "L" : "-") + ((edgeFlags & Keyboard.EDGE_RIGHT) != 0 ? "R" : "-")
					+ ((edgeFlags & Keyboard.EDGE_TOP) != 0 ? "T" : "-") + ((edgeFlags & Keyboard.EDGE_BOTTOM) != 0 ? "B" : "-"));
			return "KeyDebugFIXME(label=" + label + (shiftLabel != null ? " shift=" + shiftLabel : "") + (capsLabel != null ? " caps=" + capsLabel : "")
					+ (text != null ? " text=" + text : "") + " code=" + code + (code <= 0 || Character.isWhitespace(code) ? "" : ":'" + (char)code + "'")
					+ " x=" + x + ".." + (x + width) + " y=" + y + ".." + (y + height) + " edgeFlags=" + edges
					+ (popupCharacters != null ? " pop=" + popupCharacters : "") + " res=" + popupResId + ")";
		}
	}
	...
	public Keyboard(Context context, int defaultHeight, int xmlLayoutResId, int modeId, float kbHeightPercent) {
		...
		// Log.i("PCKeyboard", "mDefaultHeight=" + mDefaultHeight + "(arg=" + defaultHeight + ")" + " kbHeight=" + mKeyboardHeight +
	...
	/**
	 * <p>
	 * Creates a blank keyboard from the given resource file and populates it with the specified characters in left-to-right, top-to-bottom fashion, using the
	 ...
	 * @param columns the number of columns of keys to display. If this number is greater than the number of keys that can fit in a row, it will be ignored. If
	 *            this number is -1, the keyboard will fit as many keys as possible in each row.
	 */
	...
	private void setEdgeFlags() {
		if (mRowCount == 0)
			mRowCount = 1; // Assume one row if not set
		int row = 0;
		Key prevKey = null;
		int rowFlags = 0;
		for (Key key : mKeys) {
	...
	private void fixAltChars(Locale locale) {
		...
		for (Key key : mKeys) {
			...
			for (int i = 0; i < popupLen; ++i) {
				...
				if ((key.edgeFlags & EDGE_TOP) == 0 && Character.isDigit(c))
					continue;
		...
		for (int x = 0; x < gridWidth; x += mCellWidth) {
			for (int y = 0; y < gridHeight; y += mCellHeight) {
				...
				for (int i = 0; i < mKeys.size(); i++) {
					...
					if (key.squaredDistanceFrom(x, y) < mProximityThreshold || key.squaredDistanceFrom(x + mCellWidth - 1, y) < mProximityThreshold
							|| key.squaredDistanceFrom(x + mCellWidth - 1, y + mCellHeight - 1) < mProximityThreshold
							|| key.squaredDistanceFrom(x, y + mCellHeight - 1) < mProximityThreshold || isSpace
							&& !(x + mCellWidth - 1 < key.x || x > key.x + key.width || y + mCellHeight - 1 < key.y || y > key.y + key.height)) {
	...
	@Override
	public String toString() {
		return "Keyboard(" + mLayoutColumns + "x" + mLayoutRows + " keys=" + mKeys.size() + " rowCount=" + mRowCount + " mode=" + mKeyboardMode + " size="
				+ mTotalWidth + "x" + mTotalHeight + ")";
