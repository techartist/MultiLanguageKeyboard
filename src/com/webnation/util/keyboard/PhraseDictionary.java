...
/**
 * Reads a phrase dictionary and provides following-word suggestions as a list of characters for the given character.
 */
public class PhraseDictionary {
	...
	public PhraseDictionary(Context context) {
		loader = new DictionaryLoader(context.getResources().openRawResource(R.raw.dict_phrases), APPROX_DICTIONARY_SIZE, loading);
		new Thread(loader).start();
	}

	/**
	 * Returns a string containing the following-word suggestions of phrases for the given word.
	 * 
	 * @param c the current word to look for its following words of phrases.
	 * @return a concatenated string of characters, or an empty string if there is no following-word suggestions for that word.
	 */
	public String getFollowingWords(char c) {
		...
		if (index >= 0) {
			...
			int count = (index < dictionary[1].length - 1) ? (dictionary[1][index + 1] - offset) : (dictionary[2].length - offset);

			String returnableTemp = String.valueOf(dictionary[2], offset, count);
			String returnable = "";
