...
public abstract class Editor {
	...
	/**
	 * Resets the internal state of this editor, typically called when a new input session commences.
	 */
	...
	private void updateComposingText(InputConnection ic) {
		if (ic != null) {
			Log.d("", "composingText Pre0: " + composingText + "; " + ic.getTextBeforeCursor(3, 0));
			// Set cursor position 1 to advance the cursor to the text end.
			ic.setComposingText(composingText, 1);
			Log.d("", "composingText Post0: " + composingText + "; " + ic.getTextBeforeCursor(3, 0));
		}
	}

	public boolean deleteLastComposingChar(InputConnection ic, String mComposingOriginal) {
		if (hasComposingText()) {
			setComposing(mComposingOriginal.substring(0, mComposingOriginal.length() - 1));
			// Log.d("", "composingText Pre1: " + composingText + "; " + ic.getTextBeforeCursor(3, 0));
			// Delete-key are accepted only when there's text in composing.
			// composingText.deleteCharAt(composingText.length() - 1);
			// Log.d("", "composingText Post1: " + composingText + "; " + ic.getTextBeforeCursor(3, 0));
			// /updateComposingText(ic);
			return true;
		}
		return false;
	...
	public void setComposing(String composingText) {
		this.composingText = new StringBuilder();
		this.composingText.append(composingText);
	}
	 ...
	 */
	public boolean compose(InputConnection ic, int keyCode, String mComposingOriginal) {
		if (keyCode == Keyboard.KEYCODE_DELETE) {
			return deleteLastComposingChar(ic, mComposingOriginal);
		}
