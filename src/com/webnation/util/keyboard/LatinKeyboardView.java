...
import com.assistek.util.keyboard.Keyboard.Key;
...
public class LatinKeyboardView extends LatinKeyboardBaseView {
	...
	/**
	 * Whether multi-touch disambiguation needs to be disabled for any reason. There are 2 reasons for this to happen - (1) if a real multi-touch event has
	 * occured and (2) we've opened an extension keyboard.
	 */
	...
	private boolean invokeOnKey(int primaryCode) {
		getOnKeyboardActionListener().onKey(primaryCode, null, LatinKeyboardBaseView.NOT_A_TOUCH_COORDINATE, LatinKeyboardBaseView.NOT_A_TOUCH_COORDINATE);
		return true;
	}

	/**
	 * This function checks to see if we need to handle any sudden jumps in the pointer location that could be due to a multi-touch being treated as a move by
	 * the firmware or hardware. Once a sudden jump is detected, all subsequent move events are discarded until an UP is received.
	 * <P>
	 * When a sudden jump is detected, an UP event is simulated at the last position and when the sudden moves subside, a DOWN event is simulated for the second
	 * key.
	 * 
	 * @param me the motion event
	 * @return true if the event was consumed, so that it doesn't continue to be handled by KeyboardView.
	 */
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (distanceSquare > mJumpThresholdSquare && (mLastY < mLastRowY || y < mLastRowY)) {
					...
					if (!mDroppingEvents) {
						...
						// Send an up event
						MotionEvent translated = MotionEvent.obtain(me.getEventTime(), me.getEventTime(), MotionEvent.ACTION_UP, mLastX, mLastY,
								me.getMetaState());
			...
			case MotionEvent.ACTION_UP:
				if (mDroppingEvents) {
					...
					MotionEvent translated = MotionEvent.obtain(me.getEventTime(), me.getEventTime(), MotionEvent.ACTION_DOWN, x, y, me.getMetaState());
					super.onTouchEvent(translated);
					translated.recycle();
					mDroppingEvents = false;
	...
	public boolean onTouchEvent(MotionEvent me) {
		...
		if (me.getAction() == MotionEvent.ACTION_UP) {
			...
			if (languageDirection != 0) {
				getOnKeyboardActionListener().onKey(languageDirection == 1 ? KEYCODE_NEXT_LANGUAGE : KEYCODE_PREV_LANGUAGE, null, mLastX, mLastY);
		...
		if (me.getY() < 0 && (mExtensionVisible || me.getAction() != MotionEvent.ACTION_UP)) {
			if (mExtensionVisible) {
				...
				mFirstEvent = false;
				MotionEvent translated = MotionEvent.obtain(me.getEventTime(), me.getEventTime(), action, me.getX(), me.getY() + mExtension.getHeight(),
						me.getMetaState());
				...
				if (swipeUp()) {
					...
					MotionEvent cancel = MotionEvent
							.obtain(me.getDownTime(), me.getEventTime(), MotionEvent.ACTION_CANCEL, me.getX() - 100, me.getY() - 100, 0);
					super.onTouchEvent(cancel);
					cancel.recycle();
					if (mExtension.getHeight() > 0) {
						MotionEvent translated = MotionEvent.obtain(me.getEventTime(), me.getEventTime(), MotionEvent.ACTION_DOWN, me.getX(), me.getY()
								+ mExtension.getHeight(), me.getMetaState());
	...
	private void makePopupWindow() {
		...
		if (mExtensionPopup == null) {
			...
			mExtension = (LatinKeyboardView)li.inflate(mExtensionLayoutResId == 0 ? R.layout.input_trans : mExtensionLayoutResId, null);
			...
			mExtension.setOnKeyboardActionListener(new ExtensionKeyboardListener(getOnKeyboardActionListener()));
			...
			mExtensionPopup.showAtLocation(this, 0, 0, -keyboard.getHeight() + windowLocation[1] + this.getPaddingTop());
		} else {
					...
					public void handleMessage(Message msg) {
						...
						switch (msg.what) {
							...
							case MSG_TOUCH_UP:
								...
								MotionEvent me2 = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_UP, x2, y2, 0);
