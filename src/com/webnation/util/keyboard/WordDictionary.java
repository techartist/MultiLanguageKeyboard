...
/**
 * Reads a word-dictionary and provides word-suggestions as a list of characters for the specified input.
 */
public abstract class WordDictionary {
	...
	protected WordDictionary(Context context, int dictionaryId, int approxDictionarySize) {
		if (dictionaryId > -1) {
			loader = new DictionaryLoader(context.getResources().openRawResource(dictionaryId), approxDictionarySize, loading);
	 ...
	 * @return a concatenated string of characters, or an empty string if there is no word for that input.
	 */
