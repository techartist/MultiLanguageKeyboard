...
public class InputLanguageSelection extends PreferenceActivity {
	...
	private static final String[] KBD_LOCALIZATIONS = { "ar", "bg", "ca", "cs", "cs_QY", "da", "de", "el", "en", "en_DV", "en_GB", "es", "es_US", "fa", "fi",
			"fr", "fr_CA", "he", "hr", "hu", "hy", "in", "it", "iw", "ja", "ka", "ko", "lo", "lt", "lv", "nb", "nl", "pl", "pt", "pt_PT", "rm", "ro", "ru",
			"ru_PH", "si", "sk", "sk_QY", "sl", "sr", "sv", "th", "tl", "tr", "uk", "vi", "zh_CN", "zh_TW" };

	private static final String[] KBD_5_ROW = { "ar", "bg", "cs", "cs_QY", "da", "de", "el", "en", "en_DV", "en_GB", "es", "fa", "fi", "fr", "fr_CA", "he",
			"hr", "hy", "it", "iw", "lo", "nb", "pt_PT", "ro", "ru", "ru_PH", "si", "sk", "sk_QY", "sl", "sr", "sv", "th", "tr", "uk" };

	private static final String[] KBD_4_ROW = { "ar", "bg", "cs", "cs_QY", "da", "de", "el", "en", "en_DV", "fa", "fr", "fr_CA", "he", "hr", "iw", "nb", "ru",
			"ru_PH", "sk", "sk_QY", "sl", "sr", "sv", "tr", "uk" };
	...
	protected void onCreate(Bundle icicle) {
		...
		for (int i = 0; i < mAvailableLanguages.size(); i++) {
			...
			Locale locale = mAvailableLanguages.get(i).locale;
			pref.setTitle(mAvailableLanguages.get(i).label + " [" + locale.toString() + "]");
	...
	protected void onPause() {
		...
		if (checkedLanguages.length() < 1)
			checkedLanguages = null; // Save null
		SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(this);
	...
	ArrayList<Loc> getUniqueLocales() {
		...
		for (int i = 0; i < KBD_LOCALIZATIONS.length; ++i) {
			...
			if (kl.length() == 2 && langSet.contains(kl))
				continue;
			// replace zz_rYY with zz_YY
			if (kl.length() == 6)
				kl = kl.substring(0, 2) + "_" + kl.substring(4, 6);
			localeSet.add(kl);
		}
