...
/**
 * A view that renders a virtual {@link LatinKeyboard}. It handles rendering of keys and detecting key presses and touch movements. TODO: References to
 * LatinKeyboard in this class should be replaced with ones to its base class.
 * 
...
public class LatinKeyboardBaseView extends View implements PointerTracker.UIProxy {
	...
	public interface OnKeyboardActionListener {
		 ...
		 *            such as an alphabet or number or symbol, the alternatives will include other characters that may be on the same key or adjacent keys.
		 *            These codes are useful to correct for accidental presses of a key adjacent to the intended key.
		 * @param x x-coordinate pixel of touched event. If onKey is not called by onTouchEvent, the value should be NOT_A_TOUCH_COORDINATE.
		 * @param y y-coordinate pixel of touched event. If onKey is not called by onTouchEvent, the value should be NOT_A_TOUCH_COORDINATE.
		 */
	 ...
	 * 
	 * @return true if the shift is in a pressed state, false otherwise. If there is no shift key on the keyboard or there is no keyboard attached, it returns
	 *         false.
	 */
	...
	/**
	 * When enabled, calls to {@link OnKeyboardActionListener#onKey} will include key codes for adjacent keys. When disabled, only the primary key code will be
	 * reported.
	 * 
	...
	/**
	 * Requests a redraw of the entire keyboard. Calling {@link #invalidate} is not sufficient because the keyboard renders the keys to an off-screen buffer and
	...
	/**
	 * Invalidates a key so that it will be redrawn on the next repaint. Use this method if only one key is changing it's content. Any changes that affect the
	...
	/**
	 * Called when a key is long pressed. By default this will open any popup keyboard associated with this key through the attributes popupLayout and
	 * popupCharacters.
	 * 
	 * @param popupKey the key that was long pressed
	 * @return true if the long press is handled, false otherwise. Subclasses should call the method on the base class if the subclass doesn't wish to handle
	 *         the call.
	 */
	protected boolean onLongPress(Key popupKey) {
		...
		MotionEvent downEvent = generateMiniKeyboardMotionEvent(MotionEvent.ACTION_DOWN, popupKey.x + popupKey.width / 2, popupKey.y + popupKey.height / 2,
				eventTime);
