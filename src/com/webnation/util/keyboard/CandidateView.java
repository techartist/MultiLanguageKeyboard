...
public class CandidateView extends View {
	...
	public static final int MAX_CANDIDATE_COUNT = 8;
	...
	private void drawCandidates(Canvas canvas) {
		if (candidates != null) {
			...
			// if (LatinIME.mUseThisKeyboard == R.xml.kbd_qwerty_japanese) {
			// count = 1;
			// } else {
			...
			if (count > 0) {
				...
				candidateSeparator.setBounds(candidateRect[0].left, candidateRect[0].top, candidateRect[0].left + candidateSeparator.getIntrinsicWidth(),
						candidateRect[0].bottom);
				candidateSeparator.draw(canvas);
			}

			final int y = (int)(((getHeight() - paint.getTextSize()) / 2) - paint.ascent());
			for (int i = 0; i < count; i++) {
				...
				candidateSeparator.setBounds(candidateRect[i].right - candidateSeparator.getIntrinsicWidth(), candidateRect[i].top, candidateRect[i].right,
