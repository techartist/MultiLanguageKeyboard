 ...
 * <p>
 * Yes, globals are evil. But the persisted shared preferences are global data by definition, and trying to hide this by propagating the current manually just
 * adds a lot of complication. This is especially annoying due to Views getting constructed in a way that doesn't support adding additional constructor
 * arguments, requiring post-construction method calls, which is error-prone and fragile.
 * <p>
 * The comments below indicate which class is responsible for updating the value, and for recreating keyboards or views as necessary. Other classes MUST treat
 * the fields as read-only values, and MUST NOT attempt to save these values or results derived from them across re-initializations.
 * 
	...
	public void initPrefs(SharedPreferences prefs, Resources resources) {
		...
		addBooleanPref("pref_compact_mode_enabled", new BooleanPref() {
			public void set(boolean val) {
				compactModeEnabled = val;
				Log.i(TAG, "Setting compactModeEnabled to " + val);
			}

			public boolean getDefault() {
				return res.getBoolean(R.bool.default_compact_mode_enabled);
			}

			public int getFlags() {
				return FLAG_PREF_RESET_MODE_OVERRIDE;
			}
		});
		...
		addStringPref("pref_keyboard_mode_portrait", new StringPref() {
			...
			public String getDefault() {
				return res.getString(R.string.default_keyboard_mode_portrait);
			}

			public int getFlags() {
				return FLAG_PREF_RESET_KEYBOARDS | FLAG_PREF_RESET_MODE_OVERRIDE;
			}
		});
		...
		addStringPref("pref_keyboard_mode_landscape", new StringPref() {
			...
			public String getDefault() {
				return res.getString(R.string.default_keyboard_mode_landscape);
			}
		...
		addStringPref("pref_suggested_punctuation", new StringPref() {
			...
			public String getDefault() {
				return res.getString(R.string.suggested_punctuations_default);
			}
		...
		addStringPref("pref_label_scale", new StringPref() {
			public void set(String val) {
				labelScalePref = Float.valueOf(val);
			}
		...
		addStringPref("pref_candidate_scale", new StringPref() {
			public void set(String val) {
				candidateScalePref = Float.valueOf(val);
			}
		...
		addStringPref("pref_chording_ctrl_key", new StringPref() {
			public void set(String val) {
				chordingCtrlKey = Integer.valueOf(val);
			}
		...
		addStringPref("pref_chording_alt_key", new StringPref() {
			public void set(String val) {
				chordingAltKey = Integer.valueOf(val);
			}
		...
		addStringPref("pref_click_volume", new StringPref() {
			public void set(String val) {
				keyClickVolume = Float.valueOf(val);
			}

			public String getDefault() {
				return "0";// "res.getString(R.string.default_click_volume)";
			}
		...
		addBooleanPref("pref_caps_lock", new BooleanPref() {
			...
			public boolean getDefault() {
				return res.getBoolean(R.bool.default_caps_lock);
			}
		...
		addBooleanPref("pref_shift_lock_modifiers", new BooleanPref() {
			...
			public boolean getDefault() {
				return res.getBoolean(R.bool.default_shift_lock_modifiers);
			}
